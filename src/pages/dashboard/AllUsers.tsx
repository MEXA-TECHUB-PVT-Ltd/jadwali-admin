  ...
  return (
    <>
      <div className="flex justify-between items mt-10 mb-5">
        <Typography fontSize='25px'>All Users</Typography>
        <Box sx={{
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" marginTop="1rem">
        <span>Showing {((page - 1) * rowsPerPage) + 1} to {Math.min(page * rowsPerPage, users.length)} of {users.length} users</span>
        <Box>
          <Button
            variant="contained"
            disabled={page <= 1}
            ...
            sx={{
              backgroundColor: page <= 1 ? 'grey' : '#6C309C',  // Make button gray if disabled
              borderRadius: '20px',
              '&:hover': {
                backgroundColor: page <= 1 ? 'grey' : '#6C309C',  // Make button gray if disabled
              },
