...
export const WorldWideUsers = () => {
  ...
  const fetchChartUsers = async () => {
    const { res, err } = await get("/users/getByMonthCount");
    if (err) {
      console.error(err);
    }
    if (res) {
        console.log(res.counts);
        setChartUsers(res?.counts);
    }
  };
    useEffect(() => {
      fetchChartUsers();
...
const AppStatistics = ({ chartUsers }) => {
  return (
    <Card sx={{ borderRadius: "10px", flexGrow: 1 }}>
      <CardContent>
        ...
        <div className="mt-5 bg-gray-300">
          <AreaChart chartUsers={chartUsers} />
        </div>
      </CardContent>
    </Card>
  );
...
const renderCountryProgress = (countryName, value, imageSrc) => (
  <div className="mt-2 mb-5">
    ...
    <div className="relative">
      ...
      />
      <Typography className="absolute top-[-30px] right-0 text-xs text-purple-700">
        {value}%
      </Typography>
    </div>
  </div>
